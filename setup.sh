...
OS_SLUG=linux_amd64
if [ "${#DA_CHANNEL}" -eq "40" ]; then
	COMMIT="${DA_CHANNEL}"
else
	echogreen "Checking for latest build in '${DA_CHANNEL}' release channel..."
	COMMIT=$( (dig +short -t txt "${DA_CHANNEL}-version.directadmin.com" 2>/dev/null || curl --silent "https://dns.google/resolve?name=${DA_CHANNEL}-version.directadmin.com&type=txt" || curl --silent --header 'Accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=${DA_CHANNEL}-version.directadmin.com&type=txt") | sed 's|.*commit=\([0-9a-f]*\).*|\1|')
fi

if [ -z "${COMMIT}" ]; then
	echo "Unable to detect download URL. Please make there are no problems with internet connectivity, IPv6 may be configured improperly."
	exit 1
fi
...
setDAChannel() {
	if [ "${DA_CHANNEL}" != "current" ] && [ "${#DA_CHANNEL}" -ne "40" ]; then
		echogreen "Updating DA release channel in the default config..."
		${DA_PATH}/directadmin set update_channel "${DA_CHANNEL}"
		echo "action=directadmin&value=restart" >> ${DA_PATH}/data/task.queue
	fi
}
